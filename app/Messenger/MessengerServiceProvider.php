...
class MessengerServiceProvider extends ServiceProvider
{
    ...
    protected function configure()
    {
        $this->mergeConfigFrom(
            base_path('app/Messenger/config/config.php'),
            'messenger'
        );
    }
    ...
    protected function offerPublishing()
    {
        if ($this->app->runningInConsole()) {
            $this->publishes([
                base_path('app/Messenger/config/config.php') => config_path('messenger.php'),
            ], 'config');

            $this->publishes([
                base_path('app/Messenger/migrations') => base_path('database/migrations'),
            ], 'migrations');
        }
    }
